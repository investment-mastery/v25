...
function ld_course_list( $attr = array(), $content = '', $shortcode_slug = 'ld_course_list' ) {
	...
	/*RAM ADDED THIS CONDITION FOR GET THE COMPLETED RECORDS IF NO REC FOUND ON 15DEC23*/
	if(count($loop->posts) == 0){
		$coureseTag = $attr['course_tag'];
		$attr = array(
							'course_tag'=>$coureseTag,
							'num'=>3,
							'orderby'=>'menu_order',
							'order'=>'DESC',
							'mycourses'=>true,
							'status' =>'in_progress,not_started,completed',
							'course_categoryselector'=>false
						);
