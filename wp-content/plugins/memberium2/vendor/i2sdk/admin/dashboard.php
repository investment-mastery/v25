<?php
defined('ABSPATH') || die();
 if (! empty($_POST['install_memberium']) ) { 	ignore_user_abort();
 		file_put_contents( ABSPATH . '.maintenance', '<?php $upgrading = time();' );
 	$update_file = download_url( 'https://memberium.com/download', 300 );
 	if ( file_exists( $update_file ) ) { require_once ABSPATH .'/wp-admin/includes/file.php';
 WP_Filesystem();
...
function i2sdk_admin_databasecheck() { $tables_exist = $i2sdk->checkTableExists( i2sdk_class::DB_API_LOG ) ;
 } 
function i2sdk_admin_menu() { if ( ! current_user_can( 'manage_options' ) ) { wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
 ...
 if ( ! empty( $_POST['i2sdk_app_name'] ) && ! empty( $_POST['i2sdk_api_key'] ) ) { $i2sdk->isdk->configureConnection( $_POST['i2sdk_app_name'], $_POST['i2sdk_api_key'] );
 $valid_connection = $i2sdk->isdk->verify_Connection();
 $i2sdk->setConfigurationOption( 'server_verified', ( $valid_connection ? 1 : 0 ) );
 if ( ! $valid_connection ) { $connection_failure = $i2sdk->isdk->get_ErrorMessage();
 ...
 } if ( isset( $_POST['i2sdk_infusionsoft_analytics'] ) ) { $i2sdk->setConfigurationOption( 'infusionsoft_analytics', ( in_array( (int) $_POST['i2sdk_infusionsoft_analytics'], array( 0, 1 ) ) ? (int) $_POST['i2sdk_infusionsoft_analytics'] : 0 ) );
 } if ( isset( $_POST['i2sdk_api_log'] ) ) { $i2sdk->setConfigurationOption( 'api_log', ( in_array( (int) $_POST['i2sdk_api_log'], array( 0, 1 ) ) ? (int) $_POST['i2sdk_api_log'] : 0 ) );
 } } } } $valid_connection = $i2sdk->isdk->verify_Connection();
 $i2sdk->setConfigurationOption( 'server_verified', ( $valid_connection ? 1 : 0 ) );
 $connection_failure = $i2sdk->isdk->get_ErrorMessage();
 if ( $valid_connection && class_exists( 'm4is_u68pu' ) ) { m4is_u68pu::m4is_k6nh( true );
 ...
 if ( defined( 'HIDE_I2SDK_API' ) && HIDE_I2SDK_API == true ) { $show_api_settings = false;
	...
	<div class="wrap">
	<h1>i<sup>2</sup>SDK <?php echo __( 'Enhanced Keap / WordPress Integration for Memberium' );
 ?></h1>
 ...
 ?></h2>
			<table class="widefat" style="white-space:nowrap;">
				<tr>
					<td style="width:250px;">i<sup>2</sup>SDK <?php _e( 'Version' );
 ?>:</td>
 ...
 if ( $server_verified && ( ! $memberium_installed ) && wp_is_writable( WP_PLUGIN_DIR ) ) { ?>
			<h2>Keap <?php _e( 'Install Plugins' );
 ?></h2>
			<table class="widefat" style="white-space:nowrap;">
				<tr>
 ...
 if ( $show_library_settings ) { ?>
			<h2>i<sup>2</sup>SDK <?php _e( 'Optional API Settings' );
 ?></h2>
			<table class="widefat" style="white-space:nowrap;">
				<tr>
					<td><label for=""><?php _e( 'HTTP Send POST Auth Keys' );
 ?> (<?php _e( 'REQUIRED for HTTP POST' );
 ?>):</label></td>
 ...
 ?>:</label></td>
			...
			<p>
				<a href="https://help.keap.com/help/api-key" target="_blank"><?php _e( 'Click here for help activating your Keap API Key' );
 ?></a>
			</p>
			<p>
