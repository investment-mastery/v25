...
<form name="<?= $data->form_name ?>" id="<?= $data->form_name ?>" method="post">
	...
	<div>
		...
		<select name="cardtype" <?= $data->disabled ?> required="required" autocompelete="cc-type">
			<option value=""></option>
			<?php
 foreach ($data->card_types as $card_type) { $card_selected = $card_type == $data->creditcard_type ? ' selected="selected" ' : ' ';
 ...
 for ($i = 1;
 $i < 13;
 $i++) { $month_name = date('F', mktime(1, 1, 1, $i, 1, 2000) );
 ...
 for ($i = date('Y');
 $i < date('Y') + 15;
 $i++) { $selected = $data->expiration_year == $i ? ' selected="selected" ' : '';
