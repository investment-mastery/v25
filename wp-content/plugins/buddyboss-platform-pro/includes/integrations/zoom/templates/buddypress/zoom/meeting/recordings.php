...
if( get_the_ID() != 3880 && get_the_ID() != 2579 && get_the_ID() != 1249 ){
...
if (str_contains($meeting_title, 'Q&A') && !str_contains($meeting_title, 'Gold')) {
...
<div id="bp-zoom-block-show-recordings-popup-<?php echo esc_attr( $meeting_id ); ?>" class="bzm-white-popup bp-zoom-block-show-recordings mfp-hide">
	<header class="bb-zm-model-header">
		<span class="bp-meeting-title-recording-popup"><?php echo esc_attr( $meeting_title ); ?> <?php esc_html_e( '(Recordings)', 'buddyboss-pro' ); ?>&nbsp;
		 	<span>
				<div class="tooltip_1"><i class="fa fa-info-circle" aria-hidden="true"></i>
	  			<span class="tooltiptext_new">We're aware of a bug affecting Zoom Recording Playback - "Recording Freezing Issue".Our team is on it, and a fix is in the works! Thanks for your patience – we'll be back to smooth playback soon.</span>
				</div>&nbsp;
			</span>&nbsp;
		</span>
		...
		foreach ( $recordings_groups as $date => $recording_group ) {
			...
			<div class="recording-list-row-group" data-recorded-date="<?php echo esc_attr( wp_date( 'Y-m-d', strtotime( $date ) ) ); ?>">
				...
				foreach ( $recording_group as $recording ) {
					...
					<div class="recording-list-row <?php echo $recording_file->file_type;?>" data-recording-id="<?php echo esc_attr( $recording->id ); ?>">
							...
							<div class="recording-list-info">
								<h2 class="clip_title">
									...
									if ( in_array( $recording_type, array( 'shared_screen_with_speaker_view', 'shared_screen_with_gallery_view', 'active_speaker', 'shared_screen', 'shared_screen_with_speaker_view(CC)', 'gallery_view' ), true ) ) {
										esc_html_e( $meeting_title, 'buddyboss-pro' );
...
.tooltip_1:hover .tooltiptext_new {
  visibility: visible;
}
