...
<div class="zoom-meeting-block <?php echo esc_attr( $block_class_name ); ?>">
	...
	<div class="zoom-meeting-block-right">
		...
		<?php endif; ?>
		<div class="meeting-actions <?php echo 'started' === $meeting_status || ( $show_join_meeting_button && $current_date < $meeting_date_unix ) ? '' : 'bp-hide'; ?>"> <!-- bp-hide   Add after code done -->
			<?php if ( ! empty( $sign ) ) { ?>
		...
		<?php 
			...
			$show_recordings = true;
		 	...
		 	$title = preg_replace('![\s]+!u', '-', strtolower($topic));
    		$title = preg_replace('![^-\pL\pN\s]+!u', '', $title);
    		
			if (str_contains($topic, 'Q&A') && !str_contains($topic, 'Gold')) {
				$show_recordings = false;
			}elseif($title == "gold-20-monthly-accountability"){
				// echo '<a href="https://v25.investment-mastery.co/'.$title.'-replays" class="button small primary join-meeting-in-app">Show Recordings</a>';
			}else{
				echo '<a href="https://v25.investment-mastery.co/'.$title.'-replays" class="button small primary join-meeting-in-app">Show Recordings</a>';
			}
		?>
