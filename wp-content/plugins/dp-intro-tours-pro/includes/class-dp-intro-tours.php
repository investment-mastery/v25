...
class Dp_Intro_Tours {
	...
	private function define_include_hooks( $builder_mode ) {
		...
		$this->loader->add_filter( 'acf/load_value/name=trigger_object', $this, 'on_trigger_field_load', 10, 3 );
		$this->loader->add_filter( 'acf/load_value/name=dp_tour_behaviour', $this, 'on_disable_exit_on_overlay_click_field_load', 10, 3 );
	...
	private function define_admin_hooks( $plugin_public, $builder_mode ) {
		...
		if ( DP_INTRO_TOURS_IS_PRO ) {
			...
			$this->loader->add_action( 'in_plugin_update_message-dp-intro-tours-pro/index.php', $plugin_admin, 'prefix_plugin_update_message', 10, 2 );
			...
			$this->loader->add_action( 'in_plugin_update_message-dp-intro-tours/index.php', $plugin_admin, 'prefix_plugin_update_message', 10, 2 );
		...
		$this->loader->add_action( 'admin_head', $plugin_admin, 'store_transient_current_admin_color' );
	...
	public function on_disable_exit_on_overlay_click_field_load( $value, $post_id, $field ) {
		// back compatibility fix
		$oldExitOnOverlay = get_field( 'disable_exit_on_overlay_click', $post_id, false );
		if ( isset( $oldExitOnOverlay ) ) {
			Acf::update_group_field( 'dp_tour_behaviour', 'disable_exit_on_backdrop_click', $oldExitOnOverlay, $post_id );
			delete_field( 'disable_exit_on_overlay_click', $post_id );
			if ( is_array( $value ) ) {
				$value['disable_exit_on_backdrop_click'] = $oldExitOnOverlay;
			}
			return $value;
		}
		return $value;
	}
	public function on_trigger_field_load( $value, $post_id, $field ) {
		//return $value;
		$groupFieldId = ( $field['name'] === 'intro_trigger_trigger_object' ) ? 'intro_trigger' : 'intro_trigger_2';
		$selector     = Acf::get_group_field( $groupFieldId, 'intro_trigger_selector', $post_id, null );
		unset( $field['choices']['id'] );
		if ( $selector ) {
			switch ( $value ) {
				case 'id':
					if ( ! Str::starts_with( $selector, '#' ) && ! Str::starts_with( $selector, '.' ) ) {
						Acf::update_group_field( $groupFieldId, 'intro_trigger_selector', '#' . $selector, $post_id );
					}
					$value = 'custom_selector';
					Acf::update_group_field( $groupFieldId, 'trigger_object', $value, $post_id );
					break;
				case 'class':
					if ( ! Str::starts_with( $selector, '#' ) && ! Str::starts_with( $selector, '.' ) ) {
						Acf::update_group_field( $groupFieldId, 'intro_trigger_selector', '.' . $selector, $post_id );
