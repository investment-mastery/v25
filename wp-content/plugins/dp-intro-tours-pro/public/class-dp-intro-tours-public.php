...
class Dp_Intro_Tours_Public {
	 ...
	 * @since 1.0.0
	 */
	public function enqueue_styles( array $needed_extra_themes, bool $load_all_themes_override = false ) {
		...
		$this->load_needed_theme_styles( $needed_extra_themes, $load_all_themes_override );
	...
	protected function load_needed_theme_styles( array $needed_extra_themes, bool $load_all_themes_override ) {
		...
		$this->load_theme_style( $theme );
		foreach ( $needed_extra_themes as $extra_theme ) {
			$this->load_theme_style( $extra_theme );
		}
	}

	protected function load_theme_style( $theme ) {
		// light is default theme handled by core dpintro.css
		...
		if ( $is_toolbar_custom_en ) {
			...
			if ( count( $tour_to_inc ) || $add_create_new_item ) {
				...
				$wp_admin_bar->add_node(
					[
						'id'    => 'editor-menu',
						'title' => '<span class="ab-icon dpit-admin-bar-icon"></span><span class="ab-label">' . __( 'Intro Tours' ) . '</span>',
				...
				if ( $add_create_new_item ) {
					...
					$wp_admin_bar->add_node(
						[
							...
							'id'     => 'create-new-tour',
							'title'  => __( 'Create New', 'dp-intro-tours' ),
							'href'   => $createNewUrl,
						]
					);
				}
				...
				if ( count( $tour_to_inc ) ) {
					...
					foreach ( $tour_to_inc as $tourId => $boot_cfg ) {
						$tour = get_post( $tourId );
						if ( $tour ) {
							$tour_status = $tour->post_status !== 'publish' ? '<span class="dp-wpadmin-bar-tour-status"> - ' . $tour->post_status . '</span>' : '';
							...
							$this->add_tour_sub_menu_to_admin_bar(
								...
								'intro-actions',
								$tourId,
								__( 'Edit:', 'dp-intro-tours' ) . ' ' . $tour->post_title . $tour_status,
								$this->triggersCfg[ $tourId ],
								$params,
								$url_parts
							);
								$this->add_tour_sub_menu_to_admin_bar(
									...
									'edit',
									$tourId,
									__( 'Edit Intro Tour:', 'dp-intro-tours' ) . ' ' . $tour->post_title . $tour_status,
									$this->triggersCfg[ $tourId ],
									$params,
									$url_parts
								);
	...
	protected function add_tour_sub_menu_to_admin_bar( $wp_admin_bar, $parent_menu_item_id, $tour_id, $title, $triggersConfig, $url_params, $url_parts ) {
		$query_params_defs = Dp_Intro_Tours_Helper::get_dp_intro_query_params();
		$paramsSteps       = $url_params;
		$urlPartsSteps     = $url_parts;
		$paramsSteps[ Dp_Intro_Tours_Helper::get_dp_url_param_name( 'dp_qpb_origin_el_id', $query_params_defs ) ] = 'dp-step-table';
		...
		$wp_admin_bar->add_node(
			[
				...
				'title'  => __( 'Steps (visual frontend builder)', 'dp-intro-tours' ),
				'href'   => $editStepsUrl,
			]
		);
		...
		'title'   => __('Steps (admin board setup)'),
		'href'      => Path::combine_url(get_site_url(), "wp-admin/post.php?post=${tourId}&action=edit&dp_qpb_origin_el_id=dp-step-table")
		] );*/
		...
		$wp_admin_bar->add_node(
			[
				...
				'title'  => __( 'Trigger (admin board setup)', 'dp-intro-tours' ),
				'href'   => Path::combine_url(
					...
					) /* dp_to_check "?post=${tour_id}&action=edit&dp_qpb_origin_el_id=dp-trigger-config-group"*/
				),
			]
		);
		...
		$wp_admin_bar->add_node(
			[
				...
				'title'  => __( 'Design / Behavior (admin board setup)', 'dp-intro-tours' ),
			...
			if ( $triggersConfig[ $i ]['selector'] !== 'window' ) {
				...
				$wp_admin_bar->add_node(
					[
						...
						'title'  => __( 'Trigger', 'dp-intro-tours' ) . ' ' . ( $i + 1 ) . ' ' . __( 'Target', 'dp-intro-tours' ) . ' ' . __( '(visual frontend builder)', 'dp-intro-tours' ),
						'href'   => $editTriggerUrl,
					]
				);
			}
	...
	protected function enqueue_main_public_script() {
		return Dp_Intro_Tours_Enqueue::enqueue(
			'main',
			$this->builder_mode ? 'dpIntroBuilder' : 'dpIntroToursPublic',
			[
				'in_footer' => true,
				'js_dep'    => [ 'jquery' ],
			]
	...
	protected function get_tour_behaviour_cfg( $tourId, $behaviour_cfg ) {
		return [
			'exitOnOverlayClick'            => $this->builder_mode ? false : ! Arr::get( $behaviour_cfg, 'disable_exit_on_backdrop_click', false ),
			'disableStartAnimation'         => $this->builder_mode ? false : Arr::get( $behaviour_cfg, 'disable_starting_animation', false ),
			'bulletAnimSize'                => Arr::get( $behaviour_cfg, 'bullet_anim_size', 50 ),
			'scrollSpeed'                   => Arr::get( $behaviour_cfg, 'scroll_speed', 50 ),
			'overlayBlurred'                => Arr::get( $behaviour_cfg, 'overlay_blurred', false ),
			'max_tooltip_width'             => Arr::get( $behaviour_cfg, 'max_tooltip_width', '' ),
			'skip_absent_ref_el'            => $this->builder_mode ? false : ( Acf::get_group_field( 'dp_tour_behaviour', 'skip_absent_ref_el', $tourId, false, true ) ),
			'hide_previous_step_button'     => Arr::get( $behaviour_cfg, 'hide_previous_step_button', false ),
			'hide_skip_button'              => Arr::get( $behaviour_cfg, 'hide_skip_button', false ),
			'disable_navigation_by_bullets' => Arr::get( $behaviour_cfg, 'disable_navigation_by_bullets', false ),
		];
	}
	...
	protected function get_tour_labeling_cfg( $tourId ) {
		$res      = [];
		$settings = get_option( 'dp_it_labeling_options', [] );
		foreach ( $settings as $name => $value ) {
			$val_per_tour = Acf::get_group_field( 'intro_labeling', $name, $tourId );
			if ( $val_per_tour ) {
				$res[ $name ] = $val_per_tour;
			} else {
				$res[ $name ] = $value;
			}
		}
	...
	protected function assign_step_data_to_scripts( $tours_to_inc, $current_url ) {
		Dp_Intro_Tours_Helper::dp_intro_tours_console_log(
			[
		...
		if ( $tours_to_inc && count( $tours_to_inc ) ) {
			...
			$use_custom_text_styles = Arr::sget( $options, 'useThemeDefaultFont', '0' ) === '0';
			$text_style_options     = $use_custom_text_styles ? get_option( 'dp_it_text_styles_options', null ) : null;
			...
			foreach ( $tours_to_inc as $tourId => $tour_boot_cfg ) {
				...
				$this->triggersCfg[ $tourId ] = $this->get_triggers_configs( $tourId );
				if ( $shouldLoadScript || DP_INTRO_TOURS_IS_PRO ) {
					...
					if ( isset( $step_data ) ) {
						...
						$hide_el_after_start        = Arr::sget( $options, 'hide_elements_at_start', '' );
						if ( is_admin() ) {
							$hide_el_after_start = str_replace( '#wpadminbar', '', $hide_el_after_start );
						}
						$hide_el_after_start = trim( $hide_el_after_start, ' ,' );
						if ( $shouldLoadScript ) {
							...
							$tourCfg = $this->filter_tour_script_config(
								[
									...
									'tourId'              => $tourId,
									'theme'               => $theme,
									'showStepNumbers'     => Arr::sget( $options, 'hideStepNumbers', '0' ) === '0',
									'showProgress'        => Arr::sget( $options, 'showProgress', '0' ) === '1',
									'showBullets'         => Arr::sget( $options, 'hideBullets', '0' ) === '0',
									'hideElementsAtStart' => $hide_el_after_start,
									'triggers'            => $this->triggersCfg[ $tourId ],
									'steps'               => json_encode( $this->step_data[ $tourId ] ),
									'labeling'            => $this->get_tour_labeling_cfg( $tourId ),
									'behaviour'           => $this->get_tour_behaviour_cfg( $tourId, Acf::get_field( 'dp_tour_behaviour', $tourId, true ) ),
							...
							$toursCfg[] = $tourCfg;
							if ( $theme_override != 'default' && $theme_override !== Arr::sget( $options, 'theme' ) ) {
								$needed_extra_themes[] = $theme_override;
							}
						}
					}
				}
			...
			if ( $shouldLoadScripts ) {
				...
				$this->enqueue_styles( $needed_extra_themes ); // load styles just when needed
	...
	protected function get_builder_config() {
		...
		return [
			...
			'ask4ratingText'         => $is_right_time_4_ask_4_rating ? AdminNotice::get_ask_for_rating_text( floor( $activated_days / 7 ), DP_INTRO_TOURS_NAME, 'dp-intro-tours' ) : '',
			...
			'i18n'                   => [
				...
				'edit_step'                      => __( 'Edit Step %s', 'dp-intro-tours' ),
				...
				'add_after'                      => __( 'Add After', 'dp-intro-tours' ),
			...
			'adminColors'            => [
				...
				'rootSelector'     => '.dpintro-wrapper',
				'addRgbVars'       => true,
				'rgbFeatureThresh' => 300,
				'rgbDefaults'      => [
					'r' => '#E14D43',
					'g' => '#33F078',
					'b' => '#3858E9',
