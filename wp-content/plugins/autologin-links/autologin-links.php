  ...
  if (isset($_GET[PKG_AUTOLOGIN_VALUE_NAME])) {
    ...
    if ($autologin_code && pkg_autologin_check_remote_can_login()) { // Check if not empty
      ...
      if (preg_match('/^([^\?]+)\?/', $_SERVER["REQUEST_URI"], $subURIs) === 1) {
        ...
        if(isset($_REQUEST['alert_for']) && $_REQUEST['alert_for'] == "Sell") {
          $alertTypeQuery = $wpdb->prepare(
          "SELECT p.alert_id, p.profit_id, s.strategy_id, s.calc_type,s.club_type FROM `clubs_strategy_profits` p inner JOIN `clubs_strategy` s ON p.strategy_id = s.strategy_id WHERE p.profit_id = %s;",
          base64_decode($_REQUEST['detail']));
        } else {
          $alertTypeQuery = $wpdb->prepare(
          "SELECT d.alert_id, d.detail_id, s.strategy_id, s.calc_type,s.club_type FROM `clubs_strategy_details` d inner JOIN `clubs_strategy` s ON d.strategy_id = s.strategy_id WHERE d.detail_id = %s;",
          base64_decode($_REQUEST['detail']));
        }

        $resultAlertType = $wpdb->get_results($alertTypeQuery, ARRAY_A);

        if($resultAlertType[0]['calc_type'] == 'VCA' && $resultAlertType[0]['club_type']=='YTC') {
          $redirect_page = 'vca-my-alerts';
        }else if($resultAlertType[0]['calc_type'] == 'VCA' && $resultAlertType[0]['club_type']=='YCC') {
          $redirect_page = 'ycc-vca-my-alerts';
        }  
        else if($resultAlertType[0]['calc_type'] == 'CCA') {
          $redirect_page = 'cca-my-alerts';
        } else if($resultAlertType[0]['calc_type'] == 'Small Cap') {
          $redirect_page = 'small-cap-my-alerts';
        } else if($resultAlertType[0]['calc_type'] == 'CMC') {
          $redirect_page = 'cmc-my-alerts';
        }
       /* echo $redirect_page;
        echo "<pre>";
print_r($resultAlertType); exit;*/
/*echo "redirect page == ".$redirect_page; exit;*/
        // Query check code expiry
        $codeExpiryChkQuery = $wpdb->prepare(
          "SELECT * FROM `clubs_strategy_alerts` WHERE alert_id = %s  AND date_sent BETWEEN DATE_ADD(now() , INTERVAL -2 DAY) AND now();",
          $resultAlertType[0]['alert_id']); 
       /* echo $codeExpiryChkQuery;
        echo '<br>';*/
        $resultExpiryChk = $wpdb->get_results($codeExpiryChkQuery, ARRAY_A);
       
        if(count($resultExpiryChk) > 0) {
            // Query login codes
            ...
            if (count($userIds) == 1) {
              ...
              if ($userToLogin) {
                ...
                wp_redirect($protocol . '://' . $_SERVER['HTTP_HOST'] . "/". $redirect_page . $GETQuery);
                exit;
              }
            }
        } else {
         /* echo "in else block === ". $_SERVER['HTTP_HOST'];exit;*/
          $protocol = (isset($_SERVER["HTTPS"]) && ($_SERVER["HTTPS"] === "on")) ? "https" : "http";
          
          if(is_user_logged_in()) {
            $redirect_user_url = $protocol . "://" . $_SERVER['HTTP_HOST'] ."/".$redirect_page."?alert=".$_REQUEST['alert'];
          } else {
            $redirect_user_url = $protocol . "://" . $_SERVER['HTTP_HOST'] ."/login/?redirect_to=".$protocol . "://" . $_SERVER['HTTP_HOST'] ."/".$redirect_page."?alert=".$_REQUEST['alert']; 
          }          
                   
          wp_redirect($redirect_user_url);
          exit;
        }
