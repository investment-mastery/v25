...
class Lesson_Single extends Base {
	...
	public function before_get_content() {
		if ( is_singular( learndash_get_post_type_slug( 'lesson' ) ) ) {
			add_filter( 'elementor/frontend/the_content', array( $this, 'learndash_elementor_the_content' ), 10, 1 );

			add_filter( 'the_content', array( $this, 'filter_post_content' ), 30 );
		}
	...
	public function after_get_content() {
		if ( is_singular( learndash_get_post_type_slug( 'lesson' ) ) ) {
			remove_filter( 'elementor/frontend/the_content', array( $this, 'learndash_elementor_the_content' ), 10, 1 );

			remove_filter( 'the_content', array( $this, 'filter_post_content' ), 30 );
		}
