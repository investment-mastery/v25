<?php
/*
 * Template name: Course Path Club
...
.path .step .progress-main{
	position: relative;
}

.path .step .courseprogress {
    position: absolute;
    left: 15px;
    bottom: 0;
    display: -webkit-box;
...
.path .step .courseprogress canvas {
    position: absolute;
}


.path .step .courseprogress .percent {
    font-family: Source Sans Pro;
    font-style: normal;
    font-weight: 600;
    font-size: 13px;
...
<div id="primary" class="content-area bb-grid-cell">
	<main id="main" class="site-main">
		<div class="description">
			<h2><?=get_field('course_heading', $post->ID);?></h2>
			<div><?=get_field('course_description', $post->ID);?></div>
		</div>
			...
			$.ajax({ 
				...
				success: function(data){ 
					...
					$.each(data.steps, function( key,value ){
						...
						if( value.progress_percent !== null ) {
							progressCircle = `<div class="progress-main"><div class="courseprogress" data-value="${value.progress_decimal}">
                                    <span class="percent">${value.progress_percent}%</span>
                                </div></div>`;
						}
