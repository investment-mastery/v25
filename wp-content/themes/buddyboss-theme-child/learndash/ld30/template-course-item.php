...
$course_duration        = wp_get_post_terms( $course_id );
$user_course_has_access = sfwd_lms_has_access( $course_id, $current_user_id );
$course_short_description 		= get_post_meta( $course_id, '_learndash_course_grid_short_description', true );

/*RAM ADDED THS CODE TO GET COURSE DURATION ON 22DEC23*/
$course_duration 		= get_post_meta( $course_id, '_learndash_course_grid_duration', true );
if($course_duration!=''){
							$course_duration 		= $course_duration/60;
							$course_duration 		= $course_duration.' Minutes';
			     		}
/*RAM ADDED THS CODE TO GET COURSE DURATION ON 22DEC23*/
/*			     		
$course_duration_1 		= get_post_meta( $course_id );
echo '<pre>';print_r($course_duration_1);*/
//$durationInMin = $page_data->post_excerpt;
...
<li class="bb-course-item-wrap">
    ...
    <div class="bb-cover-list-item <?php echo $class; ?>">
        ...
        <div class="bb-card-course-details <?php echo ( is_user_logged_in() && isset( $user_course_has_access ) && $user_course_has_access ) ? 'bb-card-course-details--hasAccess' : 'bb-card-course-details--noAccess'; ?>">
			...
			/*RAM MODIFIED THIS CODE TO DISPLAY COURSE DURATION ON 22DEC23 -   <span style="float:right">'.$course_duration.'</span> </div>*/
			if ( $lessons_count > 0 ) {
				echo '<div class="course-lesson-count">' . $total_lessons . '<span style="float:right">'.$course_duration.'</span> </div>';
			} else {
				echo '<div class="course-lesson-count">' .
					sprintf( __( '0 %s', 'buddyboss-theme' ), LearnDash_Custom_Label::get_label( 'lessons' ) ) .
				'</div>';
			}
			/*RAM MODIFIED THIS CODE TO DISPLAY COURSE DURATION ON 22DEC23*/
			?>
