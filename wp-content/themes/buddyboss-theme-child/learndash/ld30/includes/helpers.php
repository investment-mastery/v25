 ...
 * @since 3.0.0
 *
 * @package LearnDash\LD30
 */
//echo 'Hiiiiiiiiiii234234';exit;
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
function learndash_status_bubble( $status = 'incomplete', $context = null, $echo = true ) {
	$bubble = '';
   /*RAM MODIFIED CLASS ADDED 
     in-progress-new
     Removed ld-primary-background
   */
	switch ( $status ) {
		case 'In Progress':
		case 'progress':
		case 'incomplete':
			$bubble = '<div class="ld-status ld-status-progress in-progress-new">' . esc_html_x( 'In Progress', 'In Progress item status', 'learndash' ) . '</div>';
			break;
