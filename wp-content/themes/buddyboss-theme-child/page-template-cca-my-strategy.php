<?php
/*
 * Template name: CCA My Strategy Template
 *
 */
...
<div class="container-fluid vca-cca-calculator" id="allData">
    <div class="col-lg-12 col-sm-12">
      <div class="tab_mobile_scroll">
        <div class="entry-header tab">
            <a class="tablinks tab_first" href="/cca-calculator">CCA Calculator</a>
            <a class="tablinks" href="/cca-calculator">Strategy</a>
            <a class="tablinks active" onclick="openContent(event, 'my-strategy')">My Strategies</a>
            <a class="tablinks" href="/cca-my-alerts">My Alerts <span id="alert_notify_cnt"></span></a>
        </div>
      </div>      
    </div>
    ...
    <div class="row d-none" style="display: none;" >
        <form method="Post" id="pdf_gen" action="/cca-download-excel/" target="_blank">
                ...
                <div class="content_edit" id="strategy_edit_tab_content">
                    ...
                    <div style="clear: both;">
                        <div class="edit-link-div">
                           <a href="/cca-calculator"> <button type="button" class="btn-calculate"><i class="fa fa-calculator" aria-hidden="true"></i>CCA Calculator</button></a>
...
select#currency_search{max-width: 98%;}
...
$(document).ready(function () {
    ...
    function fetch_strategy_data(calc_type = 'CCA', start_date = '', end_date = '', stock_name ='', currency = '', search_keyword = '') {
    ...
    function recalculate_strategy(strategy_id, new_investment) {
        ...
        $.each(entery_level_arr, function (index, value) {
            ...
            console.log(details_id[index+1]);             
        });
...
console.log("your_investment === "+your_investment);
    ...
    /*Update Stratergy : END */
     ...
     $('#sevieces_tbl_date').daterangepicker(
          ...
          function (start, end) {       
            ...
            fetch_strategy_data('CCA', start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'), $( "#stock_filter" ).val(), $("#currency_search").val(), $('#txtSearch').val());
          }
        ); 
    ...
    $( "#stock_filter" ).on('autocompleteselect', function (e, ui) {
        ...
        fetch_strategy_data('CCA', filter_start_date, filter_end_date, myArray[0], $("#currency_search").val(), $('#txtSearch').val());        
    });
    ...
    $('body').on('keydown', '#txtSearch', function(e) {
        if (e.which == 9 || e.which == 13) {
            ...
            fetch_strategy_data('CCA', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $("#currency_search").val(), $('#txtSearch').val());  
        }
    });

    $(document.body).on("click",".select-dropdown__list-item",function() {
        fetch_strategy_data('CCA', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $(this).data("value"), $('#txtSearch').val()); 
    });
...
function enable_checkbox(index_val) {
   ...
   function edit_cost(cnt_id){
        ...
        $("#edit-Strategy-table-body .purchase_date").each(function (index, value) {   
            ...
            console.log("#edit_invested_cost_"+cnt_val);
            ...
            if($("#edit_stock_buy_date"+cnt_val).val() != '' && $("#edit_invested_cost_"+cnt_val).val() > 0 && $("#edit_entry_cost_"+cnt_val).val() > 0 ) {
                ...
                shares_to_buy = ($("#edit_invested_cost_"+cnt_val).val() / $("#edit_entry_cost_"+cnt_val).val());
        ...
        if(sum_total_invested > 0 && total_share_buy > 0) {
            ...
            // console.log("NEW TOTAL IF");
            // console.log(new_total_inveted);
...
console.log("edit_header_total_invest :: "+new_total_inveted);        
...
console.log("-----------------------Inside edit_profit_table function....");    
    ...
    var entry_limit_val             = 50;

    for(var z=1; z <= 14; z++) {
        profit_entry_level_arr.push(entry_limit_val);
        entry_limit_val = entry_limit_val + 50;
    }
        ...
        $("#profit_tbl_body .sale_date").each(function (index, value) {   
            ...
            if($("#edit_stock_sale_date"+cnt_val).val() != '' && $("#edit_your_fiat_sale_sum_"+cnt_val).val() > 0 && $("#edit_your_take_profit_price_"+cnt_val).val() > 0) {  
                ...
                //current_holdings_prev = current_holdings; 
                    ...
                    number_of_tokens = ( sale_sum / take_profit_price );
                    tot_number_of_tokens = +(tot_number_of_tokens) + +(number_of_tokens);
                //console.log("tot_number_of_tokens === "+tot_number_of_tokens);
        ...
        $("#profit_tbl_body .sale_date").each(function (index, value) {   
            ...
            if($("#edit_stock_sale_date"+cnt_val).val() != '' && $("#edit_your_fiat_sale_sum_"+cnt_val).val() >= 0 && $("#edit_your_take_profit_price_"+cnt_val).val() >= 0) {
                ...
                if(cnt_val == 0) {
                    ...
                    var new_edit_take_profit_prices = $("#edit_take_profit_price_"+cnt_val).text();     
                    total_sale_sum = +(total_sale_sum) + +(sale_sum);

                    //number_of_tokens = ( sale_sum / take_profit_price );
                    /*poonam modified 7may*/
                    if(take_profit_price == 0.00){
                        number_of_tokens = ( sale_sum / new_edit_take_profit_prices );
                    }else{
                        number_of_tokens = ( sale_sum / take_profit_price );
                    }
    ...
    function getStrategyDetails(strategy_id) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                console.log("edit_header_total_invest :: "+data['strategy'][0]['total_investment']);
...
console.log("TOKENS");
        ...
        for (var i = 0; i < data['profits'].length; i++) {
            ...
            console.log("NEW");
                console.log(total_holding_new);
            profit_header_gain_loss = (total_holding_value + total_sale_sum) - data['strategy'][0]['your_investment'];
            profit_header_gain_loss_percent = (((total_holding_value + total_sale_sum) / data['strategy'][0]['your_investment']) * 100);
                ...
                var new_gain_loss_value = data['strategy'][0]['total_gain_loss_percentage'];    
                $("#profit_header_result").empty().text(parseFloat(new_gain_loss_value).toFixed(2)+"%");                            
                /*end poonam added 3may*/
                ...
                if(profit_header_gain_loss >= 0) {                    
                    var new_header_profit_price = data['strategy'][0]['total_gain_loss'];            
                    $("#profit_header_gain_loss").empty().html(currency_symbol+parseFloat(new_header_profit_price).toFixed(2)+" <i class='fa fa-arrow-up' aria-hidden='true'></i>");            
                    $("#profit_header_gain_loss").css('color','#67c275');
                } else {                    
                    var new_header_profit_price = data['strategy'][0]['total_gain_loss'];            
                    $("#profit_header_gain_loss").empty().html(currency_symbol+parseFloat(new_header_profit_price).toFixed(2)+" <i class='fa fa-arrow-down' aria-hidden='true'></i>");            
                    $("#profit_header_gain_loss").css('color','#67c275');
                }               
