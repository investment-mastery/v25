<?php
/*
 * Template name: Small Cap My Strategy Template
 *
 */
    ...
    <div class="col-lg-12 col-sm-12"> 
     <div class="tab_mobile_scroll">
        <div class="entry-header tab">
            ...
            <a class="tablinks" href="/small-cap-my-alerts">My Alerts <span id="alert_notify_cnt"></span></a>
        </div>
       </div>
    </div>
        ...
        <div id="my-strategy" class="container tab-pane" style="padding:0">
            <div class="col-lg-12 col-sm-12">
                <div class="search-filters">
                    ...
                    <div class="col-lg-3 col-sm-12 my-stra-text">
                        <i class="fa fa-money mr-1 stock-icon"></i> 
                        <select name="currency_search" class="form-select form-select-lg form-control" id="currency_search" placeholder-text="Currency">
                            <option value=""></option>
...
<div id="vca_popup" class="overlay">
        <div class="popup"  style="max-width:1200px !important;">
                        ...
                        <table class="table table-striped text-center" id="edit_strategy_table" style="background-color:#efefef;">
                            <thead style="text-align: center;">
                                <tr>
                                    ...
                                    <!-- <th>Average Price / Token</th> -->
                                    <th>Cost</th>
                                    <th>Alerts <span id="chek_show_all" style="display:none;"><input type="checkbox" name="checkAll" id="checkAll"></th>
                                </tr>
                            </thead>
                    ...
                    <div class="calculate">
                        ...
                        <div class="edit_header_row">
                            ...
                            <div class="edit_header_row_1 row_right edit_header_outer_right">
                                <div class="edit_header_row">
                                    <div class="edit_header_row_1 row_left edit_header_outer_div_left">
                                        <div class="edit_header_top_div">Total Amount of Tokens Sold</div>
...
$(document).ready(function () {
    ...
    function fetch_strategy_data(calc_type = 'Small Cap', start_date = '', end_date = '', stock_name ='', currency = '', search_keyword = '') {
...
$("#strategy_edit_tab_content").on("click", "#edit_header_strategy_name", function() {
    ...
    $("#edit_strategy_name_p").empty().html('Name der Strategie: <input type="text" name="edit_header_strategy_name_input" id="edit_header_strategy_name_input" value="'+old_strategy_name+'" style="width: 380px; height: 30px;">');    
});
...
$("#strategy_edit_tab_content").on("blur", "#edit_header_strategy_name_input", function() {
    ...
    $("#edit_strategy_name_p").empty().html('SName der Strategie: <span id="edit_header_strategy_name">'+new_strategy_name+'</span>');  
...
$("#strategy_profit_tab_content").on("click", "#profit_header_strategy_name", function() {
    ...
    $("#edit_strategy_name_p2").empty().html('Name der Strategie: <input type="text" name="profit_header_strategy_name_input" id="profit_header_strategy_name_input" value="'+old_strategy_name+'" style="width: 380px; height: 30px;">');    
});
...
$("#strategy_profit_tab_content").on("blur", "#profit_header_strategy_name_input", function() {
    ...
    $("#edit_strategy_name_p2").empty().html('Name der Strategie: <span id="profit_header_strategy_name">'+new_strategy_name+'</span>');  
...
function recalculate_strategy(strategy_id, new_investment) {
    ...
    $(document).on("click", "#update_stratergy", e => {
        ...
        $('#txtStrategyName').removeClass("error");      
            ...
            $("#profit_tbl_body .sale_date").each(function (index, value) {   
				...
				console.log("STR");
				console.log(store_profit_header_gain_loss);
                var store_profit_header_result = $("#profit_header_result").text().replace(/\$/g, '');

                var store_profit_header_holding_share = $("#profit_header_holding_share").text(); //ony for small-cap
                var store_profit_header_current_holding_value = $("#profit_header_current_holding_value").text(); //ony for small-cap
                var store_profit_header_total_sold = $("#profit_header_total_sold").text(); //ony for small-cap
                var store_profit_header_total_share_sell = $("#profit_header_total_share_sell").text(); //ony for small-cap
                ...
                profit_detail[index]['profit_header_holding_share'] = store_profit_header_holding_share; 
                profit_detail[index]['profit_header_current_holding_value'] = store_profit_header_current_holding_value; 
                profit_detail[index]['profit_header_total_sold'] = store_profit_header_total_sold; 
                profit_detail[index]['profit_header_total_share_sell'] = store_profit_header_total_share_sell; 
                /*end poonam added 3may*/
    ...
    /*Update Stratergy : END */
     ...
     $('#sevieces_tbl_date').daterangepicker(
          ...
          function (start, end) {       
            ...
            fetch_strategy_data('Small Cap', start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'), $( "#stock_filter" ).val(), $("#currency_search").val(), $('#txtSearch').val());
          }
        ); 
    ...
    $( "#stock_filter" ).on('autocompleteselect', function (e, ui) {
        ...
        fetch_strategy_data('Small Cap', filter_start_date, filter_end_date, myArray[0], $("#currency_search").val(), $('#txtSearch').val());        
    });
    ...
    $('body').on('keydown', '#txtSearch', function(e) {
        if (e.which == 9 || e.which == 13) {
            ...
            fetch_strategy_data('Small Cap', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $("#currency_search").val(), $('#txtSearch').val());  
        }
    });

    $(document.body).on("click",".select-dropdown__list-item",function() {
        fetch_strategy_data('Small Cap', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $(this).data("value"), $('#txtSearch').val()); 
    });
   ...
   function edit_cost(cnt_id){
        ...
        $("#edit-Strategy-table-body .purchase_date").each(function (index, value) {   
            ...
            if($("#edit_stock_buy_date"+cnt_val).val() != '' && $("#edit_invested_cost_"+cnt_val).val() > 0 && $("#edit_entry_cost_"+cnt_val).val() > 0 ) {
                ...
                shares_to_buy = ($("#edit_invested_cost_"+cnt_val).val() / $("#edit_entry_cost_"+cnt_val).val()).toFixed(5);
        ...
        /*poonam added 6may*/
        $("#edit_header_holding_share").empty().html(total_share_buy.toFixed(5));
        /*end poonam added 6may*/

        var total_token_sold = $("#profit_header_total_token_sell").val();
        var current_holding_tokens = total_share_buy_frac - total_token_sold;

        $("#edit_header_total_share_sell").empty().html(parseFloat(total_token_sold).toFixed(5));
...
function edit_profit(cnt_id) {
    ...
    var total_token_val          = 0;
    var tot_number_of_tokens    = 0;
    var tot_current_holdings    = total_tokens;
    var current_holdings_prev   = 0;
	var new_cal_total_sale_sum	= 0;
    var total_holding = 0;
    ...
    if(avg_price > 0 && total_tokens > 0) { 
        ...
        $("#profit_tbl_body .sale_date").each(function (index, value) {   
            ...
            if($("#edit_stock_sale_date"+cnt_val).val() != '' && $("#edit_your_fiat_sale_sum_"+cnt_val).val() >= 0 && $("#edit_your_take_profit_price_"+cnt_val).val() >= 0) {
                ...
                take_profit_price_new = $("#edit_take_profit_price_"+cnt_val).text();
				
                edit_profit_token_new = $("#edit_number_of_tokens_"+cnt_val).text();
				
				sale_sum_new = $("#edit_sale_sum_"+cnt_val).text();
				investment_new = $("#edit_header_total_invest").text(); //poonam added 23may
				var new_investment = investment_new.replace(/\$/g, '');  //poonam added 23may
                ...
                if(cnt_val == 0) {
					sale_sum_new = $("#edit_sale_sum_"+cnt_val).text();
					new_cal_total_sale_sum = +(new_cal_total_sale_sum) + +(sale_sum_new);
					var new_cal_gain_loss = new_cal_total_sale_sum - new_investment;
	                sale_sum = $("#edit_your_fiat_sale_sum_"+cnt_val).val();                                        
                    total_sale_sum = +(total_sale_sum) + +(sale_sum);
                    /*poonam added 15may*/
                    if(take_profit_price == 0.000000){
                        number_of_tokens = ( sale_sum / take_profit_price_new );
                    }else{
                        number_of_tokens = ( sale_sum / take_profit_price );
                    }
                    /*end poonam added 15may*/
                    ...
                    /*poonam added 23apr*/  
                    token_val = $("#edit_number_of_tokens_"+cnt_val).text();
                   
                    total_token_val = +(total_token_val) + +(token_val);  
                    /*end poonam added 23apr*/
					 $("#profit_header_gain_loss").empty().html(currencySymbol[currency]+parseFloat(new_cal_gain_loss).toFixed(2));
                } else {
                ...
                /*poonam added 15may*/
                var current_holdings = $("#edit_current_holding_"+cnt_val).text();
                var cal_sale_sum = $("#profit_header_total_sold").text();
                var live_price = $("#profit_header_live_price").text();
                var return_sale_sum = cal_sale_sum.replace(/\$/g, '');                
                var return_live_price = live_price.replace(/\$/g, '');                
                var sold_calculate_amt = current_holdings * return_live_price;

                var profit_current_holdings = $("#profit_header_holding_share").text();
                if(cnt_val >= 1){
                    var new_current_holdings = $("#edit_current_holding_"+cnt_val).text();
                    var new_sale_sum = $("#edit_sale_sum_"+cnt_val).text();
                    var sale_sum = +(new_sale_sum) + +(return_sale_sum);
                    //var current_holdings =  +(profit_current_holdings) - +(edit_profit_token_new);
                } 
                $("#profit_header_total_sold").empty().html(currencySymbol[currency]+parseFloat(sale_sum).toFixed(2));
                /*end poonam added 15may*/

                var update_current_holding_tokens = total_tokens - total_token_val;

                $("#profit_header_total_share_sell").empty().html(parseFloat(total_token_val).toFixed(5));
                $("#profit_header_holding_share").empty().html(parseFloat(update_current_holding_tokens).toFixed(5));
                $("#profit_header_current_holding_value").empty().html(parseFloat(sold_calculate_amt).toFixed(5));
                
                
                enable_checkbox(index);
                /*poonam added 3may*/
                    if(total_tokens >1) {
                        $("#profit_header_total_share_sell").empty().html(Math.abs(tot_number_of_tokens).toFixed(2));
                        $("#profit_header_holding_share").empty().html(parseFloat(current_holdings).toFixed(5));
                    }else{
                        $("#profit_header_total_share_sell").empty().html(Math.abs(tot_number_of_tokens).toFixed(5));
                        $("#profit_header_holding_share").empty().html(parseFloat(current_holdings).toFixed(5));
                    } 
                /*end poonam added 23may*/  
...
/*Edit Stratergy Data Fetching : START  */
    function getStrategyDetails(strategy_id) {
        ...
        $.ajax({
            ...
            success: (data) => {
                //console.log(data['strategy'][0]['52_week_high']); 
                ...
                /*poonam modified 8may*/
                // $("#view_header_live_price").empty().html(currency_symbol+data['strategy'][0]['stock_price']);
                // $("#edit_header_live_price").empty().html(currency_symbol+data['strategy'][0]['stock_price']);
                /*end poonam modified 8may*/
    ...
    function getStrategyProfitData(data) {
        ...
        /*poonam added 6may*/
        var profit_result_value = data['strategy'][0]['total_gain_loss_percentage'];
        var profit_result_gain_loss_value = data['strategy'][0]['total_gain_loss'];
        /*end poonam added 6may*/
        ...
        for (var i = 0; i < data['profits'].length; i++) {
            ...
            /*poonam added 20may*/
            if(data['profits'][i]['date_sale'] != '0000-00-00'){
                checked = 'disabled';
            }
            /*end poonam added 20may*/
            ...
            if(data['profits'][i]['your_take_profit_price'] == 0.000000) {
                ...
                /*end poonam added 3may*/         
            } else if(data['profits'][i]['your_take_profit_price'] != 0.000000 && data['profits'][i]['your_take_profit_price'] < 1) {
            ...
            cnt++;       
                ...
                profit_header_gain_loss = total_sale_sum - data['strategy'][0]['total_investment'];                
                ...
                $("#profit_header_live_price_input").val(data['strategy'][0]['stock_price']);
                /*end poonam modified 8may*/
                ...
                if(total_holding_value == 0){
                    $("#profit_header_result").empty().text('$0.000000%');
                }else{
                    //$("#profit_header_result").empty().text(profit_result_value+"%");
                    $("#profit_header_result").empty().text(profit_header_gain_loss_percent.toFixed(2)+"%");
                }
                /*end poonam added 6 may*/
                
                if(data['strategy'][0]['small_cap_tot_amt'] == ""){
                    ...
                    $("#profit_header_total_sold").empty().html(data['strategy'][0]['small_cap_tot_amt']);
                    $("#profit_header_total_share_sell").empty().html(data['strategy'][0]['small_cap_tot_token']);
                }

                var current_holding_tokens = total_tokens - total_token_sell;               
                /*poonam modified 20may*/
                var calculated_token = data['strategy'][0]['small_cap_curr_hold'];
                if(data['strategy'][0]['small_cap_curr_hold'] == null){
                    $("#profit_header_holding_share").empty().html(current_holding_tokens.toFixed(5));
                }else{
                    $("#profit_header_holding_share").empty().html(calculated_token);
                }
                
                var total_investment_amt = current_holding_tokens * data['live_stock_price'];
                //$("#profit_header_current_holding_value").empty().html(currency_symbol+total_holding_value.toFixed(2));
                $("#profit_header_current_holding_value").empty().html(currency_symbol+parseFloat(total_investment_amt).toFixed(2));  //poonam modified 23may
                ...
                if(profit_header_gain_loss >0){
                    $("#profit_header_gain_loss").empty().html(currency_symbol+profit_header_gain_loss+" <i class='fa fa-arrow-up' aria-hidden='true'></i>");            
                    $("#profit_header_gain_loss").css('color','#67c275');
                } else {
                    $("#profit_header_gain_loss").empty().html(currency_symbol+profit_header_gain_loss+" <i class='fa fa-arrow-down' aria-hidden='true'></i>");            
