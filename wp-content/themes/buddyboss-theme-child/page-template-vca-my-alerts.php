<?php
/*
 * Template name: VCA My Alerts Template
 *
 */
...
<div class="container-fluid vca-cca-calculator" id="allData">
    <div class="col-lg-12 col-sm-12">       
        <div class="entry-header tab">
            ...
            <a class="tablinks" href="/vca-my-strategy">My Strategies</a>
...
$(document).ready(function () {
    ...
    function fetch_alert_data(calc_type = 'VCA', start_date = '', end_date = '', stock_name ='', currency = '', alert_for = '', search_keyword = '') {
    ...
    $('#sevieces_tbl_date').daterangepicker(
          ...
          function (start, end) {       
            ...
            fetch_alert_data('VCA', start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'), $( "#stock_filter" ).val(), $("#currency_search").val(), $("#alert_search").val(), $('#txtSearch').val());
          }
        );
    ...
    $( "#stock_filter" ).on('autocompleteselect', function (e, ui) {
        ...
        fetch_alert_data('VCA', filter_start_date, filter_end_date, myArray[0], $("#currency_search").val(), $("#alert_search").val(), $('#txtSearch').val());        
    });
    ...
    $('body').on('keydown', '#txtSearch', function(e) {
        if (e.which == 9 || e.which == 13) {
            ...
            fetch_alert_data('VCA', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $("#currency_search").val(), $("#alert_search").val(), $('#txtSearch').val());  
        }
    });

    $(document.body).on("click",".select-dropdown__list--0 .select-dropdown__list-item",function() {
        fetch_alert_data('VCA', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $(this).data("value"), $("#alert_search").val(), $('#txtSearch').val()); 
        //alert("Alert for == "+$(this).data("value"));
    });

    $(document.body).on("click",".select-dropdown__list--1 .select-dropdown__list-item",function() {
        fetch_alert_data('VCA', filter_start_date, filter_end_date, $( "#stock_filter" ).val(), $("#currency_search").val(), $(this).data("value"), $('#txtSearch').val()); 
        //alert("Alert for == "+$(this).data("value"));
    });
...
$(document).on("click", "#update_stratergy", e => {
    ...
    let currencySymbol = {"USD":'&#36;', "GBP":'&pound;', "EUR":'&euro;', "AUD":'A&#36;'};
    var strategy_name = $.trim($("#edit-popup-title-1").text());
...
function enable_checkbox(index_val) {
    ...
    if(date_sale != '' && fiat_sale_sum > 0 && take_profit_price > 0 ) {  
        ...
        $("#checkAllP").attr('disabled','disabled');
    } else {
    ...
    if(avg_price > 0 && total_tokens > 0) { 
        ...
        $("#profit_tbl_body .sale_date").each(function (index, value) {   
            ...
            if($("#edit_stock_sale_date"+cnt_val).val() != '' && $("#edit_your_fiat_sale_sum_"+cnt_val).val() >= 0 && $("#edit_your_take_profit_price_"+cnt_val).val() >= 0) {
                ...
                var return_avg_price = get_average_price.replace(/[A$]/g, '');
                ...
                var return_get_total_invested_price = get_total_invested_price.replace(/[A$]/g, '');
...
/*Edit Stratergy Data Fetching : START  */
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                if(data['strategy'][0]['total_gain_loss']>1){
                   var total_gain_loss_new =  parseFloat(data['strategy'][0]['total_gain_loss']).toFixed(2);
                }else{
                     var total_gain_loss_new =  parseFloat(data['strategy'][0]['total_gain_loss']).toFixed(6);
                }
                /*end poonam added 7may*/                

                if(data['strategy'][0]['total_gain_loss'] >= 0) {

                    $("#edit_header_gain_loss").empty().html(currency_symbol+total_gain_loss_new+" <i class='fa fa-arrow-up' aria-hidden='true'></i>");            
                    $("#edit_header_gain_loss").css('color','#67c275');
                } else {
                    $("#edit_header_gain_loss").empty().html(currency_symbol+total_gain_loss_new+" <i class='fa fa-arrow-down' aria-hidden='true'></i>");            
                    $("#edit_header_gain_loss").css('color','red');
                }
    ...
    function getStrategyProfitData(data) {
        ...
        for (var i = 0; i < data['profits'].length; i++) {
            ...
            if(data['strategy'][i]['total_share_buy'] == 0) {
                var number_of_tokens_new = parseFloat(data['strategy'][i]['total_share_buy']).toFixed(2);
            } else if(data['profits'][i]['number_of_tokens'] != 0 && data['profits'][i]['number_of_tokens'] < 1) {
                var number_of_tokens_new = parseFloat(data['strategy'][i]['total_share_buy']).toFixed(6);
            } else {
                var number_of_tokens_new = parseFloat(data['strategy'][i]['total_share_buy']).toFixed(2);
            }
            /*end poonam modified 7may*/
            ...
            if(data['strategy'][0]['average_share_price']  == 0) {
                $("#checkAllP").attr('disabled','disabled');
            }else{
                $("#checkAllP").attr('enabled','enabled');
            }   
        ...
        /*end poonam added 7may*/
                ...
                /*poonam added 7may*/
                //$("#profit_header_live_price_input").val(data['live_stock_price']);
                 $("#profit_header_live_price").empty().html(currency_symbol+parseFloat(data['strategy'][0]['stock_price']).toFixed(2));
                /*end poonam added 7may*/
                ...
                if(current_holding_tokens >1) {
                    $("#profit_header_holding_share").empty().html(current_holding_tokens.toFixed(2));
                }else{
