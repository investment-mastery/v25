    ...
    <form data-current_page_url="<?php echo esc_url( $current_page_url ); ?>" id="bb-courses-directory-form" class="bb-elementor-widget bb-courses-directory" method="get" action="">
        ...
        <div class="ld-secondary-header">
            <div class="bb-secondary-list-tabs flex align-items-center" id="subnav"
                ...
                <input type="hidden" id="course-order" name="order" value="<?php echo ! empty( $_GET['order'] ) ? $_GET['order'] : 'asc'; ?>"/>
                ...
                <div class="sfwd-courses-filters flex push-right">
	                ...
	                <?php } else { ?><?php } ?> <?php //echo !empty($settings['category_filter']) && 'on' === $settings['category_filter'] ? 'active' : 'hide'; 
                     /*RAM MODIFIED ON 29NOV23*/
                    ?>
                        <!-- RAM ADDED THIS DROPDOWN ON 10JAN24 --START  -->
                        <div class="select-wrap active">
                            <?php if ( '' !== trim( buddyboss_theme()->learndash_helper()->print_wp_categories_options() ) ) { ?>
                                <select id="sfwd_cats-order-by1" name="filter-categories-assets">
                                    <?php echo buddyboss_theme()->learndash_helper()->print_wp_categories_options(); ?>
                                </select>
                            <?php } ?>
                        </div>&nbsp;
                         <!-- RAM ADDED THIS DROPDOWN ON 10JAN24 --START  -->
        ...
        <div class="grid-view bb-grid grid-box-<?php echo $course_box_border ; ?>">
            ...
            <div id="course-dir-list" class="course-dir-list bs-dir-list columns-<?php echo $course_cols; ?> <?php echo !empty( $settings['switch_media'] ) ? 'course-dir-list--media' : 'course-dir-list--hidemedia'; ?> <?php echo !empty( $settings['switch_status'] ) ? 'course-dir-list--status' : 'course-dir-list--hidestatus'; ?>">
                ...
                if ( $query->have_posts() ) {
                    ...
                    if ( ! empty( $settings['switch_pagination'] ) && 'yes' === $settings['switch_pagination'] ) : ?>
