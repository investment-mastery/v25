...
if ( ! class_exists( '\BuddyBossTheme\LearndashHelper' ) ) {
	...
	class LearndashHelper implements BBLMSHelper {
		...
		public function print_categories_options( $args = '' ) {
			/*RAM MODIFIED All Categories To Choose Your Level ON 29NOV23*/
			$defaults = [
				'selected'   => false,
				'orderby'    => 'term_id',
				'order'      => 'ASC',
				'option_all' => __( 'Choose Your Level', 'buddyboss-theme' ),
			];
			/*RAM MODIFIED All Categories To Choose Your Level ON 29NOV23*/
		 ...
		 /*RAM MODIFIED ON 10JAN2024
		  Added : New Function print_wp_categories_options
        */
		public function print_wp_categories_options( $args = '' ) {
			/*RAM MODIFIED All Categories To Choose Your Level ON 10JAN24*/
			$defaults = [
				'selected'   => false,
				'orderby'    => 'term_id',
				'order'      => 'ASC',
				'option_all' => __( 'All Asset Classes', 'buddyboss-theme' ),
			];
			/*RAM MODIFIED All Categories To Choose Your Level ON 10JAN24*/
			$args = wp_parse_args( $args, $defaults );

			if ( empty( $args['selected'] ) ) {
				$args['selected'] = isset ( $_GET['filter-categories-assets'] ) && ! empty ( $_GET['filter-categories-assets'] ) ? $_GET['filter-categories-assets'] : '';
			}

			$all_cate = "";//<option value='all'>{$args['option_all']}</option>

			$archive_category_taxonomy ='';// buddyboss_theme_get_option( 'learndash_course_index_show_categories_filter' );
			if ( empty( $archive_category_taxonomy ) ) {
				$archive_category_taxonomy = 'category';
			}
		...
		/*RAM MODIFIED THIS FUNCTION O  10JAN2024 */
		protected function _archive_filterby_tax ( $query ) {
			...
			/*RAM ADDED THIS FUNCTION ON 10JAN24 --START*/ 
			if ( ! empty( $_GET[ "filter-categories-assets" ] ) && 'all' != $_GET['filter-categories-assets'] ) {
				$archive_category_taxonomy = buddyboss_theme_get_option( 'learndash_course_index_categories_filter_taxonomy' );
				if ( empty( $archive_category_taxonomy ) ) {
					$archive_category_taxonomy = 'category';
				}
				...
				$tax_query[] = array(
					'taxonomy'         => $archive_category_taxonomy,
					'field'            => 'slug',
					'terms'            => $_GET["filter-categories-assets"],
					'include_children' => false,
				);
			}
			/*RAM ADDED THIS FUNCTION ON 10JAN24 --END*/ 
		...
		protected function _set_archive_orderby( $query ) {
			...
			$query_order_by = 'menu_order';
			$query_order    = 'asc';
			/*RAM MODIFED ON 06DEC23*/
